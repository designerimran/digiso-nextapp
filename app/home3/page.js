...
export default function Home3() {
  ...
  // --- FIXED SPLIT TEXT SECTION STARTS HERE ---
  useEffect(() => {
    if (typeof window !== 'undefined') {
      gsap.registerPlugin(SplitText);
    }
    ...
    document.fonts.ready.then(() => {
      ...
      // 2. Small delay to ensure hydration matches (Fixes Reload Error)
      timer = setTimeout(() => {
        ...
        ctx = gsap.context(() => {
          ...
          if (st.length > 0) {
            st.each((index, el) => {
              ...
              // Safety: Ensure element has text
              ...
              // Safety: Ensure chars were created
              if(!el.split.chars || el.split.chars.length === 0) return;
  ...
  // --- FIXED SPLIT TEXT SECTION ENDS HERE ---
